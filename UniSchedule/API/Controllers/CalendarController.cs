...
public class CalendarController : ControllerBase
{
    ...
    public IActionResult DownloadTestIcal()
    {
        ...
        sb.AppendLine("UID:event1-" + Guid.NewGuid().ToString() + "@unischedule.ru");
        ...
        // Событие 2 - завтра
        sb.AppendLine("BEGIN:VEVENT");
        sb.AppendLine("UID:event2-" + Guid.NewGuid().ToString() + "@unischedule.ru");
        sb.AppendLine("DTSTAMP:" + nowUtc.ToString("yyyyMMddTHHmmssZ"));
        ...
        sb.AppendLine("BEGIN:VEVENT");
        sb.AppendLine("UID:event3-" + Guid.NewGuid().ToString() + "@unischedule.ru");
        sb.AppendLine("DTSTAMP:" + nowUtc.ToString("yyyyMMddTHHmmssZ"));
    ...
    public async Task<IActionResult> ExportGroupToIcal(string groupName)
    {
        try
        {
            ...
            if (!groupSchedules.Any())
            {
                ...
                return NotFound(new { 
                    error = $"Расписание для группы {groupName} не найдено",
