...
namespace UniSchedule.API.Responses{
  public class RoomWorkloadResponse: Response{
    ...
    public async Task<RoomWorkload?> GetRoomWorkload(){
      ...
      List<TeacherSchedule> schedule = await _o_api.SendRequest<TeacherSchedule>(
    ...
    private async Task<Dictionary<string, Dictionary<int, bool>>> convertToScheduleDictionary(List<TeacherSchedule> teacherSchedules){
      ...
      foreach (var dateGroup in groupedByDate){
          Dictionary<int, bool> slots = new Dictionary<int, bool>(){
            { 1, false },
            { 2, false },
            { 3, false },
            { 4, false },
          ...
          foreach (var schedule in dateGroup){
              int slotIndex = Array.IndexOf(Const.ALL_SLOTS, schedule.slot) + 1;
