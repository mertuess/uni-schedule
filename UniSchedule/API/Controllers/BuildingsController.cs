...
public class BuildingsController : ControllerBase
{
    ...
    private readonly OutAPI _oApi;

    public BuildingsController(OutAPI oApi, JsonParser jsonParser, CacheService cache)
    {
        _oApi = oApi;
    ...
    public async Task<IActionResult> GetBuildings()
    {
        ...
        var buildings = await _oApi.SendRequest<Building>("/buildings", "buildings");
    ...
    {
        var key = $"schedule:building:{bui_id}:{start}:{end}";
        if (_cache.TryGet<object>(key, out var cached))
            return Content(_jsonParser.Serialize(cached), "application/json");

        var response = new BuildingWorkloadResponse(_oApi, bui_id, new Week(start, end));
    ...
    {
        var key = $"schedule:buildings:{bui_ids}:{start}:{end}";
        if (_cache.TryGet<object>(key, out var cached))
            return Content(_jsonParser.Serialize(cached), "application/json");

        var ids = bui_ids.Split(',', StringSplitOptions.RemoveEmptyEntries).Select(int.Parse).ToArray();
        var response = new BuildingsWorkloadResponse(_oApi, ids, new Week(start, end));
